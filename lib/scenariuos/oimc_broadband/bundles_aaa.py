...
def optional(type_,exclude_,telco_codes_):
    ...
    if exclude_[2] == 'True':result+= f""" and not {type_}_client_address <<= any(select oinp_subnet from oims.oper_ip_numbering_plan_history where oinp_oper_id in ({telco_codes_}) and oinp_description ilike '%Служебн%')"""
    if exclude_[2] == 'True':result+= f""" and not {type_}_server_address <<= any(select oinp_subnet from oims.oper_ip_numbering_plan_history where oinp_oper_id in ({telco_codes_}) and oinp_description ilike '%Служебн%')"""
    return result
